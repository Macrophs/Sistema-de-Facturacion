...
export const validateForm = async (data, type) => {
  ...
  if (data.name.length === 0) errors.name = "El nombre es requerido";
  ...
  if (type === "employee" || type === "client") {
    let id = 0;
    if (type === "client" && data.id_client) {
      id = data.id_client;
    } else if (type === "employee" && data.id_user) {
      id = data.id_user;
    }
    ...
    if (data.cedula.length === 0) errors.cedula = "La cédula es requerida";
