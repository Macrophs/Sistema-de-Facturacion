    ...
    {   //retornar vista donde se selecciona el método de pago
        return (
            <>
                ...
                <section className={`mt-10 p-5 w-full md:w-1/2 rounded-md  flex items-center justify-center flex-col group transition ease-in-out hover:bg-marianBlue cursor-pointer  ${method === 1 ? "bg-marianBlue " : " " } `} onClick={()=> setMethod(1)}>
                        ...
                        <h1 className={`mt-4 text-marianBlue text-center font-bold text-xl group-hover:text-white ${method === 1 ? "text-white " : "text-marianBlue " }`}>Punto de Venta</h1>
                    </section>

                    <section className={`mt-10 p-5 w-full md:w-1/2 rounded-md  flex items-center justify-center flex-col group transition ease-in-out hover:bg-marianBlue cursor-pointer  ${method === 2 ? "bg-marianBlue " : " " } `} onClick={()=> setMethod(2)}>
                        ...
                        <h1 className={`mt-4 text-marianBlue text-center font-bold text-xl group-hover:text-white ${method === 2 ? "text-white " : "text-marianBlue " }`}>Efectivo</h1>
                    </section>

                    <section className={`mt-10 p-5 w-full md:w-1/2 rounded-md  flex items-center justify-center flex-col group transition ease-in-out hover:bg-marianBlue cursor-pointer  ${method === 3 ? "bg-marianBlue " : " " } `} onClick={()=> setMethod(3)}>
                            ...
                            <h1 className={`mt-4 text-marianBlue text-center font-bold text-xl group-hover:text-white ${method === 3 ? "text-white " : "text-marianBlue " }`}>Pago Móvil</h1>
                    </section>
                </section>
            
                <section className="flex items-center justify-center pt-10">
                    <button className="p-2 bg-red-500 text-white font-bold text-center  rounded-lg text-1xl px-4 py-2.5 mr-2 mb-2" onClick={() => onClose()} > Cancelar</button>
                    <button disabled={method === null ? true : false} className={` ${method === null ? "bg-gray-400 opacity-20" : " bg-marianBlue" }  text-white font-bold text-center  rounded-lg text-1xl px-4 py-2.5 mr-2 mb-2`} onClick={()=> setPage(2)}> Continuar</button>
                </section>
            ...
            <>
               ...
               <section className="flex justify-center items-center  flex-col mt-5 text-xl">
                    ...
                    <section className="p-2">
                        ...
                        <p className="pt-2"><span className="text-marianBlue font-bold">Total a Pagar: </span> <span className="text-green-500"> 10000$ </span></p>
                ...
                <section className="flex items-center justify-center pt-10">
                    <button className="p-2 bg-red-500 text-white font-bold text-center  rounded-lg text-1xl px-4 py-2.5 mr-2 mb-2" onClick={() => onClose()} > Cancelar</button>
                    <Link href={"factura"} className={` ${!checked ? "bg-gray-400 opacity-20 pointer-events-none" : " bg-marianBlue" } p-2 text-white font-bold text-center  rounded-lg text-1xl px-4 py-2.5 mr-2 mb-2 `} onClick={()=> setPage(2)}> Terminar Compra</Link>
